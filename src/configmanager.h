...
public:
	inline static constexpr auto ININAME = "CostlierTempering";

	static ConfigManager& getInstance() {
		static ConfigManager instance(std::format("Data/SKSE/Plugins/{}.ini", ININAME).c_str());
		return instance;
	}

	bool HasKey(const std::string& key, const std::optional<std::string> setToIfNotExist = std::nullopt) {
		bool exist = cfg[ININAME].isKeyExist(key);
	...
	bool Modify(const std::string& Key, const std::string& Value, const std::string& comment = "") {
		return cfg.modify(ININAME, Key, Value, comment);
	}

	std::string GetKey(const std::string& Key) {
		return cfg[ININAME][Key];
	}
